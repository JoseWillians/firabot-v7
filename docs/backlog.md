...
- [ ] Definir API administrativa inicial. Motivo: painel precisa consultar status do bot, documentos, links e setores sem mexer direto no banco. Arquivos provaveis: futura pasta `src/admin` ou futuro serviço administrativo. Status: prioridade atual.
...
- [ ] Preparar painel administrativo com RBAC por setor. Motivo: administrador principal deve controlar o bot e administradores setoriais devem editar apenas suas áreas. Arquivos provaveis: `docs/RELATORIO_ESTRATEGICO.md`, `database/schema.sql`, futura API/painel. Status: prioridade atual.
