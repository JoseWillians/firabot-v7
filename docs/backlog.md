...
- [ ] Confirmar fluxo completo no codigo. Motivo: README descreve fluxo, mas opcoes finais dependem do codigo. Arquivos provaveis: `src/menus`, `src/commands`, `src/middlewares/messageHandler.ts`, `src/services`. Status: pendente.
...
- [x] Fazer PPC do Curso listar cursos antes dos PPCs disponíveis. Motivo: preparar múltiplos cursos e manter PPCs atuais apenas em Engenharia de Computação. Arquivos provaveis: `src/menus/courseMenu.ts`, `src/middlewares/messageHandler.ts`. Status: primeira etapa concluida.
- [ ] Carregar documentos por categoria/setor no banco. Motivo: DRCA, CAE, Biblioteca e PPC devem crescer sem hardcode. Arquivos provaveis: `database/schema.sql`, `src/functions/database.ts`, `src/services/documentService.ts`. Status: futuro.
- [ ] Preparar painel administrativo com RBAC por setor. Motivo: administrador principal deve controlar o bot e administradores setoriais devem editar apenas suas áreas. Arquivos provaveis: `docs/RELATORIO_ESTRATEGICO.md`, `database/schema.sql`, futura API/painel. Status: melhoria futura.
...
- [ ] Extrair testes com socket fake para `messageHandler`. Motivo: reduzir risco antes de refatorar flows e submenus. Arquivos provaveis: `tests/`, `src/middlewares/messageHandler.ts`. Status: proximo passo recomendado.
...
- [ ] Confirmar TTL de estado. Motivo: `USER_STATE_TTL_MINUTES` pode estar planejado mas nao aplicado. Arquivos provaveis: `src/services/userStateService.ts`. Status: pendente de confirmacao.
- [ ] Confirmar uso de `ADMIN_NUMBERS`. Motivo: pode ser preparacao futura. Arquivos provaveis: `.env.example`, `src/commands`. Status: pendente de confirmacao.
